...
class _NotifState extends State<Notif> {
  ...
  Future<void> _loadNotifications() async {
    ...
    final storedNotifications = prefs.getStringList('notifications') ?? [];
    final newNotifications = storedNotifications.map((text) {
      final id = storedNotifications.indexOf(text); // Unique ID for each notification
      return NotificationItem(id: id, text: text, isRead: prefs.getBool('isRead_$id') ?? false);
    }).toList();
  ...
  void _navigateToScreen(NotificationItem item) async {
    ...
    if (item.id == 'New data are added please check your Direct Bill') {
      Get.to(() => const Direct_bill());
    } else if (item.id == 'New data are added please check your Radiology Bill') {
      Get.to(() => const Radiology());
    } else if (item.id == 'New data are added please check your Pharmacy Bill') {
      Get.to(() => const Pharmacy());
    } else if (item.id == 'New data are added please check your pathology Bill') {
      Get.to(() => const Pathalogy());
    } else if (item.id == 'New data are added please check your Ambulance Bill') {
      Get.to(() => const Ambulance());
    } else if (item.id == 'New data are added please check your Blood_bank Bill') {
      Get.to(() => const Blood_Bank());
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.lightBlue[50],
      appBar: AppBar(
        title: Text('Notification (${notifications.where((item) => !item.isRead).length} Unread)'),
        centerTitle: true,
        backgroundColor: darkYellow,
      ),
      body: ListView.builder(
...
class NotificationItem {
  ...
  NotificationItem({required this.id, required this.text, this.isRead = false});
}

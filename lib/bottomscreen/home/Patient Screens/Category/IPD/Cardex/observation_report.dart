...
class Observationreport extends StatefulWidget {
  const Observationreport({Key? key}) : super(key: key);

  @override
  State<Observationreport> createState() => _ObservationreportState();
}

class _ObservationreportState extends State<Observationreport> {
  ...
  Future<void> fetchMedicationreport() async {
    ...
    if (response.statusCode == 200) {
      ...
      final MedicationreportList =
          jsonResponse['observation_report'] as List<dynamic>;
      ...
      setState(() {
        Medicationreport = MedicationreportList.map((MedicationItem) {
          return {
            ...
            "bp": MedicationItem['bp'],
            "p": MedicationItem['p'],
            "r": MedicationItem['r'],
            "tem": MedicationItem['tem'],
          };
        }).toList();
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text('Bp: ${MedicationItem["bp"] ?? 'N/A'}'),
                              Text(
                                  'Respiration: ${MedicationItem["r"] ?? 'N/A'}'),
                              Text(
                                  'Tempreture: ${MedicationItem["tem"] ?? 'N/A'}'),
                            ],
                          ),
                        ),
                      );
